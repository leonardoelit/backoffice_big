...
import { GameStat } from '../constants/types';
...
const GameReportsTable = () => {
  ...
  const [sortField, setSortField] = useState<keyof GameStat | ''>('');
  ...
  const getStats = async (range: { MinCreatedLocal: string; MaxCreatedLocal: string }) => {
    setIsLoading(true);
    const result = await getGameStats({ timeStampFrom: range.MinCreatedLocal, timeStampTo: range.MaxCreatedLocal });
    
    if (!result.isSuccess) {
      showToast(result.message ?? "Oyun raporları yüklenirken hata!", "error");
      setIsLoading(false);
      return;
    }
    ...
    if (result.gameStats) {
      startTransition(() => {
        setStats(result.gameStats);
        setCurrentPage(1); // Reset page on new data
      });
    } else {
      showToast("Oyun raporları yüklenirken hata!", "error");
    }
    setIsLoading(false);
  };
  ...
  // Sorting handler
  const handleSort = (field: keyof GameStat) => {
    if (sortField === field) {
  ...
  // --- Provider Color Logic (Unchanged) ---
          ...
          <div className="min-h-[400px]"> {/* Reduced min-height */}
            <Table>
              <TableHeader className="bg-gray-50 dark:bg-slate-800/50">
                <TableRow className="border-b border-gray-200 dark:border-slate-800">
                  ...
                  <TableCell
                    ...
                    className="px-5 py-3.5 text-left text-sm font-semibold text-gray-600 dark:text-gray-400"
                  >
                    {/* This one is not sortable in the original, so no onClick */}
                    Oyun
                  </TableCell>
              ...
              <TableBody className={`divide-y divide-gray-100 dark:divide-slate-800 transition-opacity duration-300 ${isLoading ? 'opacity-50' : 'opacity-100'}`}>
                {isLoading ? (
                  Array.from({ length: 5 }).map((_, i) => <SkeletonRow key={i} columns={6} />) // Use a fixed number for skeleton
                ) : (
                  paginatedStats.map((t) => (
                    <TableRow key={`${t.providerId}-${t.gameName}`} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition-colors">
                      <TableCell className="px-5 py-4 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap" >{t.providerId}</TableCell>
                      <TableCell className={`px-5 py-4 text-sm font-medium ${getProviderColor(t.providerId)} whitespace-nowrap`}>{t.providerName}</TableCell>
                      <TableCell className="px-5 py-4 text-sm text-gray-800 dark:text-gray-200">{t.gameName}</TableCell>
                ...
                {!isLoading && stats.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-10 text-gray-500 dark:text-gray-400">
                      No game reports found for the selected date range.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
