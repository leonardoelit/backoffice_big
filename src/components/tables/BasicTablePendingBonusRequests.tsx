              ...
              {loading ? (
                ...
                bonusRequests.map((t:BonusData) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell className={`px-4 py-3 ${t.status === 1 || t.status == 2 ? 'text-yellow-600' : t.status === 3 ? 'text-success-600' : t.status === 4 ? 'text-red-600' : 'text-gray-500 dark:text-gray-400'} text-start text-theme-sm`}>
