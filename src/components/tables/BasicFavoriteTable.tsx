...
const confirmRemove = async () => {
  ...
  try {
    ...
    if(res.isSuccess){
      showToast("Oyuncu favorilerden kaldırıldı", "success");
      handleRefetch();
    } else {
      showToast(res.message ?? "Oyuncuyu favorilerden kaldırırken hata", "error");
    }
  } catch (error) {
    console.log(error);
    showToast("Oyuncuyu favori listesinden kaldırırken hata", "error");
  }
  setIsRemoving(false);
...
{open && (
  <div
    className="absolute mt-2 w-full md:w-[40vw] max-w-4xl right-0 
...
export default BasicFavoriteTable